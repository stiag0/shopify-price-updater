// shopify-updater.js
// Consolidated Shopify Price and Inventory Updater with 2025-04 API compatibility
...
// Generate a new log filename for each run based on timestamp
const timestamp = new Date().toISOString().replace(/:/g, '-').replace(/\..+/, '');
const LOG_FILE_PATH = process.env.LOG_FILE_PATH || path.join('logs', `shopify-sync_${timestamp}.log`);
// Keep max size limit for safety if needed
...
const Logger = {
    ...
    async rotateLog() {
        // Since we're generating a new log file for each run, we'll simplify this function
        // to just check the log size and output a warning if it gets too large
        ...
        try {
            ...
            const stats = await fs.promises.stat(this.logPath);
            if (stats.size >= LOG_MAX_SIZE_BYTES) {
                const warningMessage = `Log file has exceeded maximum size of ${LOG_MAX_SIZE_MB}MB. Consider increasing LOG_MAX_SIZE or check for issues causing excessive logging.`;
                console.warn(`[${new Date().toISOString()}] [WARN] ${warningMessage}`);
                await fs.promises.appendFile(this.logPath, `[${new Date().toISOString()}] [WARN] ${warningMessage}\n`, 'utf8');
            }
        } catch (error) {
            console.error(`Error checking log size for ${this.logPath}: ${error.message}`);
        }
...
/**
 * Loads discount data from CSV file or URL
 */
async function loadDiscounts(csvPath) {
    return new Promise(async (resolve, reject) => {
        ...
        if (isUrl) {
          try {
            Logger.log(`🌐 Downloading discount CSV from URL: ${csvPath}`);
            const resp = await axios.get(csvPath, { responseType: 'stream' });
            inputStream = resp.data;
          } catch (err) {
            return reject(new Error(`Error downloading CSV: ${err.message}`));
          }
        } else {
          // Local path
          if (!fs.existsSync(csvPath)) {
            return reject(new Error(`Discount CSV file does not exist at ${csvPath}`));
          }
        ...
        inputStream
          ...
          .on('end', () => {
            Logger.log(`🗒️ Loaded ${discounts.size} discounts from ${isUrl ? 'URL' : 'local file'}`);
            resolve(discounts);
          })
 ...
 * Uses quantities field for 2025-04+ API compatibility
...
async function getAllShopifyVariants() {
    ...
    // Updated query using the 2025-04 API schema
...
/**
 * Fetches the GID of the first active inventory location.
 ...
 * Updated for 2025-04 API compatibility.
 * 
...
async function updateVariantInShopify(variant, newPrice, newInventory, locationId) {
    ...
    // Try to get quantity from quantities field (2025-04 API)
...
async function syncShopifyData() {
    ...
    let fetchedLocationId = null;
    
    // Try to load discounts if available
    let discountMap = new Map();
    try {
        if (fs.existsSync(DISCOUNT_CSV_PATH) || /^https?:\/\//i.test(DISCOUNT_CSV_PATH)) {
            discountMap = await loadDiscounts(DISCOUNT_CSV_PATH);
        } else {
            Logger.warn(`Discount CSV file not found at ${DISCOUNT_CSV_PATH}, continuing without discounts.`);
        }
    } catch (e) {
        Logger.error(`Error loading discounts from ${DISCOUNT_CSV_PATH}`, e);
    }
    ...
    try {
        ...
        // Process items with discounts first
        ...
        if (withDiscount.length > 0) {
            Logger.log(`Processing ${withDiscount.length} SKUs with discounts first, then ${withoutDiscount.length} without discounts.`);
            stats.withDiscount = withDiscount.length;
        }
            ...
            if (SYNC_TYPE === 'price' || SYNC_TYPE === 'both') {
                ...
                if (!isNaN(basePrice)) {
                    ...
                    if (discountMap.has(sku)) {
                        ...
                        Logger.log(`💸 SKU ${sku}: Applying ${pct}% discount: ${basePrice} → ${finalPrice.toFixed(2)}`);
                    }
        ...
        Logger.log(`Shopify API Version: ${SHOPIFY_API_VERSION}`);
        Logger.log(`---------------------------`);
        Logger.log(`Total Items Considered: ${itemsToProcess.length}`);
        Logger.log(`Items with Discounts: ${stats.withDiscount}`);
